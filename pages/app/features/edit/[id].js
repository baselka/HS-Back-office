...
const Index = () => {
  ...
  const _getFeatureDetails = () => {
    // Api.Features.details(id).then((res)=>{
    //   setLoadingData(false);
    //   NotificationManager.success('تم تحديث الميزة ', 'نجاح', 3000);
    // });
  }

  const _updateFeature = data => {
    Api.Features.update(data).then((res)=>{
      setLoadingData(false);
      NotificationManager.success('تم تحديث الميزة ', 'نجاح', 3000);
    });
  }
  ...
  return (
    <Container>
        <Layout>
          ...
          <div>
          <Widget title="تحديث بيانات ميزة" description={""} >
              <form
