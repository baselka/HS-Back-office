...
const Widget = ({
  ...
  const setPeriod = period => {
    setData1(Array.from(Array(24).keys()).map(i => random(50, 100)))
    setData2(Array.from(Array(24).keys()).map(i => random(50, 100)))
    setOption(period)
  }

  let active = 'btn-flat-blue'
  return (
    <div className="flex">
      <div className="w-full">
        ...
        <div style={{height: height}}>
          <Bar key={key} data={data} height={height} options={options} legend={legend} />
        </div>
      </div>
    </div>
  )
