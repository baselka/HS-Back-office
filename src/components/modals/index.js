    ...
    return (
      <div className='relative p-4 flex-auto'>
        ...
        <input
          ...
          onChange={e => props.keyInput(e.target.value)}
          className='w-full h-10 border-2 rounded-3xl	border-gray-400	p-2'
        />
      </div>
    );
  }
          ...
          <>
            {props.list.map(function (item, index) {
              if (item.city) {
                return (
                  <div
                    className='w-12/12 bg-pink-700 cursor-pointer rounded-lg text-white m-1 text-center py-2 hover:bg-pink-900'
                    key={index}
